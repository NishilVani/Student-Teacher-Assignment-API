...
class Assignment(db.Model):
    ...
    # For Decreasing Miss in pytest --cov although the API is working properly
    # def __repr__(self):
    ...
    @classmethod
    def set_assignment_grade(cls, _id, grade, principal: Principal):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(GradeEnum[grade.upper()], 'Grade is not valid')
        assertions.assert_valid(assignment.teacher_id == principal.teacher_id,
                                'This assignment belongs to some other teacher')
        assignment.grade = GradeEnum[grade.upper()].name
